...
from median_2dcd import MSFgNet
...
my_epoch = 100
my_batch_size = 1
training_samples =  1469 #2586 #1451 # 88831, 4217, 1182 , 951 , 1451 , 302 , 433, 266, 4252 , 1951 , 48285 , 5953, 5753
epoch_resume = 0
...
dataset = {
            ...
            'baseline':['highway', 'PETS2006'], 									#, 'pedestrians', 'office'
            'cameraJitter':['badminton', 'boulevard'], 								#, 'sidewalk', 'traffic'
            'dynamicBackground':['boats', 'fall', 'fountain01'], 							# 'canoe',, 'fountain02', 'overpass'
            'intermittentObjectMotion':['abandonedBox', 'parking', 'sofa'], 	#, 'streetLight', 'tramstop', 'winterDriveway'
            'lowFramerate':['tramCrossroad_1fps', 'turnpike_0_5fps'], 		#, 'tunnelExit_0_35fps','port_0_17fps', 
            'nightVideos':['bridgeEntry','fluidHighway','tramStation'],		#,'busyBoulvard',,'streetCornerAtNight', 'winterStreet'
            'PTZ':['continuousPan', 'twoPositionPTZCam'], 											#, 'intermittentPan', 'zoomInZoomOut'
            'shadow':['backdoor', 'busStation', 'copyMachine'], 												#, 'bungalows', , 'cubicle''peopleInShade'
            'thermal':['diningRoom', 'lakeSide', 'library'], 												#, '', 'park''corridor'
            'turbulence':['turbulence0', 'turbulence3'] 											#, 'turbulence2', 'turbulence1',
}
"""
...
#        label_path_add = folder+mode+'_output/*.png'
    ...
    """
    X = np.zeros((batch_size,depth, size_shape[1], size_shape[0]), dtype='float32')
    main_X = np.zeros((batch_size, 1, size_shape[1], size_shape[0]), dtype='float32')  
    M = np.zeros((batch_size, 1, size_shape[1], size_shape[0]), dtype='float32') 
    Y = np.zeros((batch_size, 1, size_shape[1], size_shape[0]), dtype='float32')    
    ...
    while True:
        ...
        for category in dataset_1:   
            ...
            for scene in scene_list:
                ...
                for i,(image_path, label_path, median_path) in enumerate(zip(image_path_list[49:stop],label_path_list[49:stop],median_path_list[:stop])):
...
path = r'C:\Users\santo\OneDrive\Desktop\soumendu_work\data\model_save\2dcd_median_murari_50p\cameraJitter\BW_init\*h5' 
...
if (files):
    ...
    input_shape3 = (1, my_size_shape[1], my_size_shape[0])
    model = MSFgNet(input_shape, input_shape2, input_shape3)
...
checkpoint = ModelCheckpoint(r'C:\Users\santo\OneDrive\Desktop\soumendu_work\data\model_save\2dcd_median_murari_50p\cameraJitter\BW_init\dsfmdl.{epoch:03d}.h5', monitor='loss', mode='min')
...
#saving model
#model.save('/home/cvprlab/backgroud-subtraction2/data/dataset_2014/model_save/calc_ch_last/wet_snow_ab1/final_dsfmdl.h5')
#del model
